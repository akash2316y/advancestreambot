...
class Var(object):
    ...
    BOT_TOKEN = str(getenv('BOT_TOKEN', '7626180904:AAGlwYhTH0gV-6HwGWUl_m6E8IkAv1AyLwo'))
    name = str(getenv('name', 'filetolinkbot'))
    SLEEP_THRESHOLD = int(getenv('SLEEP_THRESHOLD', '60'))
    WORKERS = int(getenv('WORKERS', '4'))
    BIN_CHANNEL = int(getenv('BIN_CHANNEL', '-1002616161685'))
    PORT = int(getenv('PORT', 8080))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', '152.53.155.85'))
    PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
    OWNER_ID = set(int(x) for x in os.environ.get("OWNER_ID", "8110231942").split())  
    ...
    FQDN = str(getenv('FQDN', 'https://152.53.155.85')) if not ON_HEROKU or getenv('FQDN') else APP_NAME + '.herokuapp.com'
    HAS_SSL = bool(getenv('HAS_SSL', False))

    if HAS_SSL:
        URL = "https://152.53.155.85/".format(FQDN)
    else:
        URL = "https://152.53.155.85/".format(FQDN)
